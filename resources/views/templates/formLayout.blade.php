...
@php
    if (isset($edit) && !empty($edit)){
        $method='PUT';
        $url = route('houses.update' , ['house' => $house->id]);
        $product= 'MODIFICA';
    }else{
        $method='POST';
...
<div class="form-container">
    <hr>
    <h3 class="new-title">{{$product}} INSERISCI IMOBILE</h3>
    <hr>
    <form class="needs-validation" action="{{$url}}" method="post" novalidate>
        ...
        <div class="form-group my-form">
            ...
            <input class="form-control {{$errors->has('image') ? 'is-invalid' : '' }}" type="text" name="image" placeholder="URL" value="{{isset($house) ? $house->image : ''}}" required>
            <div class="invalid-feedback">
                {{$errors->first('image')}}
            </div>
        </div>
